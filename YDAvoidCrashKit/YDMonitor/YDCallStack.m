...
typedef struct YDStackFrame {
    const struct YDStackFrame *const previous;
    const uintptr_t return_address;
} YDStackFrame;

//存储 thread 信息的结构体
typedef struct YDThreadInfoStruct {
    double cpuUsage;
    integer_t userTime;
} YDThreadInfoStruct;

static mach_port_t _ydMainThreadId;

@implementation YDCallStack

+ (void)load {
    _ydMainThreadId = mach_thread_self();
}

+ (NSString *)YDCallStackWithType:(YDCallStackType)type {
    if (type == YDCallStackTypeAll) {
        //组成的数组
        ...
        for (int i = 0; i < thread_count; i++) {
            //当前执行的指令
            [reStr appendString:YDStackOfThread(threads[i])];
        }
        NSString *memStr = @"";
        ...
        return [reStr copy];
    }else if (type == YDCallStackTypeMain) {
        //主线程
        NSString *reStr = YDStackOfThread((thread_t)_ydMainThreadId);
        assert(vm_deallocate(mach_task_self(), (vm_address_t)_ydMainThreadId, 1 * sizeof(thread_t)) == KERN_SUCCESS);
        NSLog(@"%@",reStr);
        return [reStr copy];
    } else {
        //当前线程
        ...
        [nsthread setName:originName];
        reStr = YDStackOfThread(mach_thread_self());
        NSLog(@"%@",reStr);
        return [reStr copy];
    }
}

NSString *YDStackOfThread(thread_t thread) {
    //thread info
    YDThreadInfoStruct threadInfoSt = {0};
    ...
    mach_msg_type_number_t state_count = YDThreadStateCountByCPU();
    kern_return_t kr = thread_get_state(thread, YDThreadStateByCPU(), (thread_state_t)&machineContext.__ss, &state_count);
    if (kr != KERN_SUCCESS) {
        return [NSString stringWithFormat:@"Fail get thread: %u", thread];
    }
    //通过指令指针来获取当前指令地址
    const uintptr_t instructionAddress = YDMachInstructionPointerByCPU(&machineContext);
    buffer[i] = instructionAddress;
    ++i;
    
    //
    uintptr_t linkRegisterPointer = YDMachThreadGetLinkRegisterPointerByCPU(&machineContext);
    ...
    YDStackFrame stackFrame = {0};
    //通过栈基址指针获取当前栈帧地址
    const uintptr_t framePointer = YDMachStackBasePointerByCPU(&machineContext);
    if (framePointer == 0 || YDMemCopySafely((void *)framePointer, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
        return @"Fail frame pointer";
    }
    for (; i < 32; i++) {
        buffer[i] = stackFrame.return_address;
        if (buffer[i] == 0 || stackFrame.previous == 0 || YDMemCopySafely(stackFrame.previous, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
            break;
        }
    }
    ...
    YDSymbolicate(buffer, symbolicated, stackLength, 0);
    for (int i = 0; i < stackLength; ++i) {
        [reStr appendFormat:@"%@", YDOutputLog(i, buffer[i], &symbolicated[i])];
    }
    [reStr appendFormat:@"\n"];
    return reStr;
}
...
NSString* YDOutputLog(const int entryNum, const uintptr_t address, const Dl_info* const dlInfo) {
...
kern_return_t YDMemCopySafely(const void *const src, void *const dst, const size_t byteSize) {
    vm_size_t bytesCopied = 0;
    return vm_read_overwrite(mach_task_self(), (vm_address_t)src, (vm_size_t)byteSize, (vm_address_t)dst, &bytesCopied);
}

#pragma mark - Symbolicate
void YDSymbolicate(const uintptr_t* const stackBuffer, Dl_info* const symbolsBuffer, const int stackLength, const int skippedEntries) {
    int i = 0;
    if (!skippedEntries && i < stackLength) {
        YDDladdr(stackBuffer[i], &symbolsBuffer[i]);
        i++;
    }
    for (; i < stackLength; i++) {
        YDDladdr(YDInstructionAddressByCPU(stackBuffer[i]), &symbolsBuffer[i]);
    }
}
bool YDDladdr(const uintptr_t address, Dl_info* const info) {
    info->dli_fname = NULL;
    info->dli_fbase = NULL;
    info->dli_sname = NULL;
    info->dli_saddr = NULL;
    //根据地址获取是哪个 image
    const uint32_t idx = YDDyldImageIndexFromAddress(address);
    if (idx == UINT_MAX) {
        return false;
    }
    /*
    ...
    const uintptr_t segmentBase = YDSegmentBaseOfImageIndex(idx) + imageVMAddressSlide;
    if (segmentBase == 0) {
        return false;
    }
    //
    ...
    uintptr_t cmdPointer = YDCmdFirstPointerFromMachHeader(machHeader);
    if (cmdPointer == 0) {
        return false;
    }
...
uint32_t YDDyldImageIndexFromAddress(const uintptr_t address) {
...
uintptr_t YDCmdFirstPointerFromMachHeader(const struct mach_header* const machHeader) {
...
uintptr_t YDSegmentBaseOfImageIndex(const uint32_t idx) {
    const struct mach_header* machHeader = _dyld_get_image_header(idx);
    //查找 segment command 返回 image 的地址
    uintptr_t cmdPtr = YDCmdFirstPointerFromMachHeader(machHeader);
    if (cmdPtr == 0) {
        return 0;
    }
 ...
 */
uintptr_t YDMachStackBasePointerByCPU(mcontext_t const machineContext) {
...
uintptr_t YDMachInstructionPointerByCPU(mcontext_t const machineContext) {
...
uintptr_t YDInstructionAddressByCPU(const uintptr_t address) {
#if defined(__arm64__)
...
uintptr_t YDMachThreadGetLinkRegisterPointerByCPU(mcontext_t const machineContext) {
#if defined(__i386__)
    return 0;
#elif defined(__x86_64__)
    return 0;
